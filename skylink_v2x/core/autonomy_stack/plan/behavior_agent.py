...
from skylink_v2x.core.misc import get_speed, positive, cal_distance_angle
from skylink_v2x.core.autonomy_stack.plan.collision_check import CollisionChecker
from skylink_v2x.core.autonomy_stack.plan.local_planner_behavior import LocalPlanner
from skylink_v2x.core.autonomy_stack.plan.global_route_planner import GlobalRoutePlanner
from skylink_v2x.core.autonomy_stack.plan.global_route_planner_dao import GlobalRoutePlannerDAO
from skylink_v2x.core.autonomy_stack.plan.planer_debug_helper import PlanDebugHelper
...
class BehaviorAgent(object):
    ...
    def __init__(self, vehicle, carla_map, config_yaml, comm_manager):

        self.vehicle = vehicle
        self._agent_id = vehicle.id
    ...
    def update_information(self, ego_pos, ego_speed, objects):
        ...
        # for obj in objects:
        #     print("obj: ", obj['actor'].get_location())
        #     print("blueprint: ", obj['actor'].type_id)
        ...
        # TODO: 
        self.objects = {
            'vehicles': [obj['actor'] for obj in objects if obj['is_dynamic']],
            'traffic_lights': [obj['actor'] for obj in objects if obj['is_traffic_light']],
        }
        ...
        # TODO: Here we should not have full object information. Instead, all we know is the bounding box of the object.
    ...
    def run_step(
        ...
        if self.traffic_light_manager(ego_vehicle_wp) != 0:
            # return 0, None
            self._comm_manager.buffer_planning(self._agent_id, (0, None))
        ...
        if car_following_flag:
            ...
            self._comm_manager.buffer_planning(self._agent_id, (target_speed, target_loc))
