    ...
    def takeoff_and_ascend(self, takeoff_target: airsim.Vector3r, vehicle: Optional[carla.Actor] = None) -> None:
        ...
        while True:
            ...
            if self.latest_camera_image is not None:
                # Visualize the camera image in a window.
                # cv2.imshow("Drone Camera View", self.latest_camera_image)
                # cv2.waitKey(1)
                # Save the image if needed.
                filename = os.path.join('/home/xiangbog/Folder/Research/SkyLink/skylink/debug', f"{self.latest_frame:06d}.png")
                cv2.imwrite(filename, self.latest_camera_image)
